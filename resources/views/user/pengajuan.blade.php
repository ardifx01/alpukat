...
foreach (($syaratPengawas ?? []) as $s) {
if ($errors->has("dokumen.$s->id")) { $activeTab = 'pengawas'; break; }
}
if (request('tab') === 'pengurus') { $activeTab = 'pengurus'; }
if (request('tab') === 'pengawas') { $activeTab = 'pengawas'; }
...
$hasPengawas = !empty($syaratPengawas) && count($syaratPengawas) > 0;
@endphp
  ...
  <div class="container py-4" style="max-width:1040px;">
    ...
    <div class="card shadow-sm border-0 mb-4" style="border-radius:18px; overflow:hidden;">
      ...
      <div class="px-3 px-md-4 pt-3 pb-3">
        @if($onCooldown)
        <div class="alert alert-warning border-0 shadow-sm rounded-3 mb-2 small" id="cooldownBox">
          <i class="fa fa-clock-o me-1"></i>
          Anda bisa mengunggah lagi <span id="cooldownTimer">{{ $cooldownUntil->diffForHumans(['parts' => 2]) }}.
        </div>
        @endif
      ...
      <div class="card-header bg-white border-0 pb-0">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
          <ul class="nav nav-pills gap-2 mb-0" id="uploadTab" role="tablist">
            ...
            <li class="nav-item" role="presentation">
              <button class="nav-link soft-pill {{ $activeTab==='pengawas' ? 'active' : '' }}"
                id="pengawas-tab" data-bs-toggle="tab" data-bs-target="#pane-pengawas" type="button" role="tab">
                Pengawas Koperasi
              </button>
            </li>
          </ul>
          ...
          <div class="d-flex align-items-center gap-2 x-small">
            ...
            <span class="chip" id="chipPengurus"><i class="fa fa-circle me-1"></i> Pengurus Koperasi: <b>Belum</b></span>
            <span class="chip" id="chipPengawas"><i class="fa fa-circle me-1"></i> Pengawas Koperasi: <b>Belum</b></span>
          </div>
        </div>
      </div>
          ...
          <div class="tab-content">
            ...
            <div class="tab-pane fade {{ $activeTab==='koperasi' ? 'show active' : '' }}" id="pane-koperasi" role="tabpanel">
              ...
              <div class="mt-3 d-flex justify-content-end">
                <button type="button" class="btn btn-outline-primary btn-sm px-3 btn-next">
                  Selanjutnya <i class="fa fa-arrow-right ms-1"></i>
                </button>
              </div>
              <div class="bar-warning x-small text-danger mt-2 d-none">
                Lengkapi unggahan pada <strong>Koperasi</strong> sebelum kirim.
              </div>
            </div>
            ...
            <div class="tab-pane fade {{ $activeTab==='pengurus' ? 'show active' : '' }}" id="pane-pengurus" role="tabpanel">
              @if(!$hasPengurus)
              <div class="alert alert-secondary rounded-3 small mb-2">Belum ada persyaratan untuk kategori pengurus koperasi.</div>
              @else
              <div class="row g-2">
              ...
              <div class="bar-warning x-small text-danger mt-2 d-none">
                Lengkapi unggahan pada <strong>Pengurus Koperasi</strong> sebelum kirim.
              </div>
            </div>

            {{-- === PANE PENGAWAS === --}}
            <div class="tab-pane fade {{ $activeTab==='pengawas' ? 'show active' : '' }}" id="pane-pengawas" role="tabpanel">
              @if(!$hasPengawas)
              <div class="alert alert-secondary rounded-3 small mb-2">Belum ada persyaratan untuk kategori pengawas koperasi.</div>
              @else
              <div class="row g-2">
                @foreach ($syaratPengawas as $syarat)
                @php $inputId = "dokumen_pengawas_{$syarat->id}"; @endphp
                <div class="col-12 col-md-6 col-lg-6">
                  <div class="p-2 rounded-3 border upload-card h-100">
                    <div class="small text-dark">
                      <strong>{{ $syarat->nama_syarat }}</strong>
              ...
              <div class="bar-warning x-small text-danger mt-2 d-none">
                Lengkapi unggahan pada <strong>Pengawas Koperasi</strong> sebelum kirim.
              </div>
            </div>
          </div>
        </form>
...
<style>
  ...
  .dz {
    background: #f9fbff;
    border: 1px dashed #c8d2ff !important;
    cursor: pointer;
    font-size: .85rem;
    transition: .15s
  }
...
<script>
  (function() {
    ...
    function updateChips() {
      const k = catStatus('koperasi'),
        p = catStatus('pengurus'),
        w = catStatus('pengawas');
      const ck = document.getElementById('chipKoperasi');
      const cp = document.getElementById('chipPengurus');
      const cw = document.getElementById('chipPengawas');
      if (ck) {
        ck.classList.toggle('ok', k.ok);
        ck.querySelector('b').textContent = k.ok ? 'Lengkap' : 'Belum';
      }
      if (cp) {
        cp.classList.toggle('ok', p.ok);
        cp.querySelector('b').textContent = p.ok ? 'Lengkap' : 'Belum';
      }
      if (cw) {
        cw.classList.toggle('ok', w.ok);
        cw.querySelector('b').textContent = w.ok ? 'Lengkap' : 'Belum';
      }
    }

    function updateSubmit() {
      const okK = catStatus('koperasi').ok;
      const okP = catStatus('pengurus').ok;
      const okW = catStatus('pengawas').ok;
      const allOk = okK && okP && okW;

      const btn = document.getElementById('btnKirim');
      if (btn) btn.disabled = !allOk;

      // Toggle SEMUA BAR warning per tab
      document.querySelectorAll('.bar-warning')
        .forEach(el => el.classList.toggle('d-none', allOk));
    ...
    document.getElementById('formPengajuan')?.addEventListener('submit', function(e) {
      ...
      // 1) semua kategori harus OK, termasuk pengawas
      const allOk = ['koperasi', 'pengurus', 'pengawas'].every(cat => catStatus(cat).ok);
      if (!allOk) {
        e.preventDefault();

        // 2) cari input wajib pertama yang belum ada file-nya
        const firstErrReq = Array.from(document.querySelectorAll('input[type="file"][data-required="1"]'))
          .find(inp => !(inp.files && inp.files.length));

        if (firstErrReq) {
          // (opsional, UX) pindahkan ke tab yang benar dulu
          const pane = firstErrReq.closest('.tab-pane');
          if (pane) {
            const trigger = document.querySelector(`[data-bs-target="#${pane.id}"]`);
            if (trigger && window.bootstrap?.Tab) new bootstrap.Tab(trigger).show();
          }

          // lalu scroll ke dropzone-nya
          document.querySelector(`label.dz[for="${firstErrReq.id}"]`)
            ?.scrollIntoView({
              behavior: 'smooth',
              block: 'center'
            });
...
<script>
  (function() {
    const until = new Date("{{ $cooldownUntil->toIso8601String() }}");
    const span = document.getElementById('cooldownTimer');

    function update() {
      const now = new Date();
      let diff = Math.floor((until - now) / 1000);
      if (diff <= 0) {
        span.textContent = "sekarang 🎉";
        document.getElementById('btnKirim')?.removeAttribute('disabled');
        clearInterval(timer);
        return;
      }
      ...
      let text = [];
      if (d) text.push(d + " hari");
      if (h) text.push(h + " jam");
      if (m) text.push(m + " menit");
      if (s) text.push(s + " detik");
